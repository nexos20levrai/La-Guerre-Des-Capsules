...
func _on_player_disconnected(player):
	...
	warning_label.text = player.name + " s'est déconnecté."
...
func _on_player_killed(attacker_id,shot_type,victim_id):
	...
	if shot_type == Scores.HEADSHOT:
		text = "[color=%s]%s[/color] a tué [color=%s]%s[/color] d'un tir à la tête !" % [attacker_color,attacker_name,victim_color,victim_name]
	elif shot_type == Scores.TAILSHOT:
		text = "[color=%s]%s[/color] a tué [color=%s]%s[/color] d'une balle dans les fesses !" % [attacker_color,attacker_name,victim_color,victim_name]
	else:
		text = "[color=%s]%s[/color] a tué [color=%s]%s[/color]." % [attacker_color,attacker_name,victim_color,victim_name]
...
func _on_player_kill_streak(id,kills):
	...
	match kills:
		...
		10:
			text = "Personne n'arrêtera [color=%s]%s[/color] ? Dix kills déjà !" % [color,player_name]
		15:
